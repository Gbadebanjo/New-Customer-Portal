 ...
 *
 * Protects NOC work: if an existing row has status='verified' (sent to
 * the customer) OR status='in_progress' (edited by a NOC user and
 * awaiting Send Report), it's left untouched even if AMMP now returns
 * different numbers. Only untouched 'raw' rows are overwritten.
 */
export async function ingestReportDataForCustomer(customerId, siteIds, { from, to, token }) {
    ...
    for (const siteId of siteIds) {
        ...
        for (const [dateKey, vals] of perDay.entries()) {
            ...
            if (existing && (existing.status === 'verified' || existing.status === 'in_progress')) {
                skipped++;
                continue;
            }
