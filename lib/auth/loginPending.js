...
// then a build-time-only fallback that WILL warn if hit at request time.
function getSigningSecret() {
    return process.env.LOGIN_PENDING_SECRET
...
function verify(token) {
    ...
    const expected = crypto.createHmac('sha256', getSigningSecret()).update(b64).digest('base64url');
