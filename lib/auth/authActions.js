...
import { setLoginPending } from "@/lib/auth/loginPending";
...
// Round up to the nearest whole minute, but never below 1 — so a wait
// of "40 seconds left" shows as "1 minute" instead of "0 minutes".
...
export async function login(formData) {
    ...
    if (ipCheck.limited) {
        ...
        // Show the ACTUAL remaining wait, not the full window duration.
        // Otherwise a user who checks at minute 14 still sees "15 minutes"
        // and it looks like the timer is resetting on every attempt.
        const remainingLabel = formatDuration(minutesFromMs(ipCheck.retryAfterMs));
        return {
            errors: {
                email: `You've reached ${maxAttempts} failed attempts. Try again in ${remainingLabel}.`,
            },
        };
    }
    ...
    if (existingUser.is_locked_out) {
        ...
        // Compute remaining time from the ORIGINAL lockout start, so the
        // countdown ticks down on each check instead of always showing
        // the full window (which reads as "the timer keeps restarting").
        // Falls back to full duration if we somehow don't have a start
        // time — same behaviour as before.
        const startedAt = existingUser.lockout_started_at
            ? new Date(existingUser.lockout_started_at)
            : (existingUser.lockout_until
                ? new Date(new Date(existingUser.lockout_until).getTime() - durationMs)
                : new Date());
        const remainingMs = startedAt.getTime() + durationMs - Date.now();
        const remainingLabel = remainingMs > 0
            ? formatDuration(minutesFromMs(remainingMs))
            : durationLabel;
        return {
            errors: {
                email: `You've reached ${maxAttempts} failed attempts. Try again in ${remainingLabel}.`,
    ...
    try {
        ...
        if (!isValidPassword) {
            // Sliding-window failure counter: if the last failed attempt
            // landed more than `durationMs` ago (or never happened),
            // reset the count to 1. Otherwise increment. Everything is
            // done in one atomic query so concurrent failed attempts
            ...
            const [rows] = await sequelizeConnection.query(
                `UPDATE users SET
                    failed_login_attempts = CASE
                        WHEN last_failed_login_at IS NULL OR last_failed_login_at < :cutoff THEN 1
                        ELSE failed_login_attempts + 1
                    END,
                    last_failed_login_at = :now
                 WHERE id = :id
                 RETURNING failed_login_attempts`,
                { replacements: { id: existingUser.id, cutoff, now } }
            );
        ...
        await sequelizeConnection.query(
            `UPDATE users SET failed_login_attempts = 0, is_locked_out = false, lockout_until = NULL, lockout_started_at = NULL, last_failed_login_at = NULL WHERE id = :id`,
            { replacements: { id: existingUser.id } }
        );

        // Clear the per-IP throttle counter too. Without this, a
        // legitimate user who fumbled their password a couple of times
        // (or reset it, or hit the lock, then finally succeeded) leaves
        // the IP counter high — and the very next login attempt from
        // the same network (a different tab, a colleague on shared
        // Wi-Fi, or the same person on another browser) hits the "3
        // attempts" wall on its first try. resetRateLimit is idempotent
        // and cheap; call it every time password verification succeeds.
        ...
        if (existingUser.totp_enabled) {
            ...
            // Signed cookie binds the pending userId to this session so
            // downstream `verify2FA` calls can't be pointed at a
            // different user by a malicious client.
            await setLoginPending(existingUser.id, '2fa');
            return {
                require2FA: true,
                user: {
                    id: existingUser.id,
        ...
        // Same reasoning: bind pending userId to this login attempt so
        // startEnable2FA / enable2FAAndLogin can't be pivoted onto a
        // different (non-2FA) account.
        await setLoginPending(existingUser.id, 'setup');
...
export async function resetPassword(formData) {
    ...
    try {
        ...
        await sequelizeConnection.query(
            `UPDATE users SET email_confirmed = true, is_locked_out = false, failed_login_attempts = 0, lockout_until = NULL, lockout_started_at = NULL, last_failed_login_at = NULL WHERE id = :id`,
