 ...
 *
 * `_ignoredUserId` is kept for backwards compatibility with existing
 * callers but is IGNORED — the trusted user id comes from the session.
 * `assetIds` are intersected against the caller's authorised sites so
 * a Customer user can't ask about a site their customer doesn't own.
 */
export async function getCompareData(_ignoredUserId, assetIds, windowDays = 7) {
    const { user } = await verifyAuth();
    if (!user?.id) return { sites: [] };
    ...
    // Filter down to sites this user is actually allowed to see. Fleet
    // roles pass every id through; customer roles are narrowed to
    // their mapped sites.
    const allowed = await intersectWithAuthorized(userId, assetIds.map(String), 'asset_id');
    // intersectWithAuthorized accepts strings or objects — we passed
    // strings, so it returns the same string list filtered.
    assetIds = allowed;
    if (assetIds.length === 0) return { sites: [] };
