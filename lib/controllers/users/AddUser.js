...
// Roles that grant write privileges. Mirrors the guard in
// lib/controllers/users/updateUserById.js — only a top-tier `Admin`
// may create a user with either of these. A Portal Admin creating a
// user cannot elevate them (or by proxy themselves through a shared
// account) to Admin or Portal Admin.
...
export default async function AddUser(formData) {
    try {
        ...
        const timezone = sanitizeTimezone(xss(formData.get('Timezone')));
        ...
        // Elevation guard — only Admin may create an Admin or Portal Admin user.
        if (!callerIsAdmin && roles.some((r) => ELEVATED_ROLE_NAMES.has(r?.name))) {
            return { error: 'Only an Admin can create a user with the Admin or Daystar Portal Admin role.' };
        }
        ...
        if (sendInvitation) {
            ...
            await logSecurityEvent({ action: 'InvitationSent', userId: id, userName: email, ip: ctx.ip, browserInfo: ctx.browserInfo });
        }
