...
export default async function enable2FAAndLogin(userId, token) {
    ...
    if (!valid) {
        await logSecurityEvent({ action: 'TwoFactorSetupFailed', userId: String(userId), userName: user.email, ip: ctx.ip, browserInfo: ctx.browserInfo });
    ...
    await logSecurityEvent({ action: 'LoginSucceeded', userId: String(userId), userName: user.email, ip: ctx.ip, browserInfo: ctx.browserInfo, extra: { method: '2FA-setup' } });
